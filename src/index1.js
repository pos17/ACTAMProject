...
async function propagateStateChanges(isFirst) { 
  ...
  if(state.drawing.chords.isChanged) {
    console.log()
    if(state.drawing.chords.playingPart) {
      console.log("here I am")
      state.drawing.chords.playingPart.stop()
      Tone.Transport.stop()
      //state.drawing.chords.playingPart.cancel(0)
    } else {
      console.log("here I am not")
    }
    Tone.Transport.loopEnd = "4:0:0";
    console.log(state.drawing.chords.instrument)
    state.drawing.chords.playingPart =playChordSequence(state.drawing.chords.sequence, state.key, new Instr.Pad())    /*constructInstrument(state.drawing.chords.instrument)*/
    if(!isFirst) {
